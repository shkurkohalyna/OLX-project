...
import fetchFavouritesDelete from './fetch/fetchFavouritesDelete';
...
function onFavouritesClicked(){
    ...
    console.log("add");
    fetchFavourites(API_OLX, currentItem._id).then(updateFavourites)
...
fetchGetFavorites(API_OLX).then(setFavourites)
    for(const item of itemsData){
        if(item.imageUrls[0]===undefined)continue;
        if(item.imageUrls[0].toString() === event.target.src){
            showModal(item);
            console.log(item);
            currentItem=item;
        }
...
function addHoverText(){
    ...
    salerInfo.insertAdjacentHTML('afterbegin', '<div class="nav-and-description-button-hovered"><p data-item-modal-salerinfo class="nav-and-description-button-hovered-bold">Ольга</p><p data-item-modal-salerinfo>- на OLX с нояб. 2018</p><p data-item-modal-salerinfo class="nav-and-description-button-hovered-bold">'+currentItem.phone+'</p></div>');
}
...
function addNonhoverText(){
    removeSalerInfoChilds();
    salerInfo.insertAdjacentHTML('afterbegin', '<p class="nav-and-description-button-nonhovered">Информация о<br> продавце</p>');
}
